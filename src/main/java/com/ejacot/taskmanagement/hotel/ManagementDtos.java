...
public final class ManagementDtos{
 ...
 public record RoomAssignmentView(Long id,Long employeeId,String employee,LocalDate date,String roomNumber,String category,RoomAssignmentStatus status,String notes,String defectDescription,String checkedBy,java.time.Instant checkedAt,java.time.Instant completedAt,DefectStatus defectStatus,java.time.Instant defectResolvedAt,String defectResolvedBy,boolean hasDefectPhoto){static RoomAssignmentView from(RoomAssignment value){return new RoomAssignmentView(value.getId(),value.getEmployee().getId(),value.getEmployee().getUsername(),value.getWorkDate(),value.getRoomNumber(),value.getCategory(),value.getStatus(),value.getNotes(),value.getDefectDescription(),value.getCheckedBy()==null?null:value.getCheckedBy().getUsername(),value.getCheckedAt(),value.getCompletedAt(),value.getDefectStatus(),value.getDefectResolvedAt(),value.getDefectResolvedBy()==null?null:value.getDefectResolvedBy().getUsername(),value.hasDefectPhoto());}}
}
