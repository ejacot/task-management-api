...
public class CheckerService {
 private final UserAccountRepository users;private final RoomAssignmentRepository rooms;private final DeliveryService delivery;
 public CheckerService(UserAccountRepository users,RoomAssignmentRepository rooms,DeliveryService delivery){this.users=users;this.rooms=rooms;this.delivery=delivery;}
 ...
 public ManagementDtos.RoomAssignmentView review(String username,Long id,CheckerDtos.RoomReview r){UserAccount checker=user(username);RoomAssignment room=rooms.findByIdAndHotelId(id,hotel(checker)).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND));if(r.status()==RoomAssignmentStatus.ASSIGNED)throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Alege CHECKED, DEFECT sau RELEASED");room.review(checker,r.status(),r.notes(),r.defectDescription());String title=switch(r.status()){case CHECKED->"Camera verificata";case DEFECT->"Problema la camera "+room.getRoomNumber();case RELEASED->"Camera eliberata";default->"Camera actualizata";};delivery.notifyInApp(room.getEmployee(),title,room.getRoomNumber()+" · "+r.status(),"rooms");return ManagementDtos.RoomAssignmentView.from(room);}
 public ManagementDtos.RoomAssignmentView defect(String username,Long id,CheckerDtos.DefectUpdate r){UserAccount checker=user(username);RoomAssignment room=rooms.findByIdAndHotelId(id,hotel(checker)).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND));if(room.getStatus()!=RoomAssignmentStatus.DEFECT)throw new ResponseStatusException(HttpStatus.CONFLICT,"Camera nu are defect deschis");if(r.photoData()!=null&&!r.photoData().isBlank())room.attachDefectPhoto(r.photoName()==null?"defect-photo":r.photoName(),r.photoData());if(r.status()==DefectStatus.IN_PROGRESS)room.startDefect(checker,r.notes());else if(r.status()==DefectStatus.RESOLVED)room.resolveDefect(checker,r.notes());else if(r.status()!=DefectStatus.OPEN)throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Status defect invalid");delivery.notifyInApp(room.getEmployee(),"Defect actualizat la camera "+room.getRoomNumber(),r.status().name(),"rooms");return ManagementDtos.RoomAssignmentView.from(room);}
 private CheckerDtos.DayRooms summary(LocalDate date,List<RoomAssignment> items){return new CheckerDtos.DayRooms(date,items.size(),count(items,RoomAssignmentStatus.ASSIGNED),count(items,RoomAssignmentStatus.COMPLETED),count(items,RoomAssignmentStatus.CHECKED),count(items,RoomAssignmentStatus.DEFECT),count(items,RoomAssignmentStatus.RELEASED),items.stream().map(ManagementDtos.RoomAssignmentView::from).toList());}
