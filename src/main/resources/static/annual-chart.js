(() => {
  ...
  function availableYears() {
    if (!annualData?.logs) return [];
    return [...new Set(annualData.logs.map(log => Number(log.date.slice(0, 4))))].sort((a, b) => b - a);
  }
  ...
  function renderAnnualChart() {
    ...
    const hourlyRate = Number(annualData.me.hourlyRate || 0);
    const values = Array(12).fill(0);
    annualData.logs.filter(log => Number(log.date.slice(0, 4)) === selectedYear)
